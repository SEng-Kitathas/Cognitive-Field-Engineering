# CFE ACTIVE TRACE MATRIX

As of: 2026-08-31 08:28 Eastern Daylight Time

| Claim | Evidence | Status |
|---|---|---|
| Experience arrangement matters | v1.0 family-specific matched-content effects | bounded load-bearing phenomenon |
| Identifying support matters | v1.1 +17.56pt mean predicate delta, 5/6 positive | bounded load-bearing construction lesson |
| Identifying support solves strict predicate | v1.1 support slices contradict | rejected |
| Factorized primitives guarantee composition | v1.2 aggregate | rejected |
| Simple mixed optimizer windows cause instability | v1.3 seeds3101-3103 | weakened interim, not closed |
| Curator decomposition may differ from learner decomposition | v1.2-v1.3 dissociation | candidate law, unproven |
| CFE is mature predictive engineering | no evidence yet | not earned |
| CFE program remains viable | repeated mechanism-specific effects + falsifiable next branches | provisional but strengthening as research program |

| Geometry matters at program level | v1.0 matched-content arrangement effects + v1.1 support geometry + v1.3 seed3104 phenotype divergence | bounded load-bearing interpretation |
| Any chosen geometry is beneficial | repeated sign reversals across v1.0-v1.3 | rejected |
| AI core analogy | architecture-level analogy to persistent structured developmental field | explanatory only, not scientific claim |

| StarMap historical lineage exists broadly | partial E-drive audit: 215 substantive files / 175 unique contents / ~4402 occurrences | verified bounded finding; exhaustive coverage not earned |
