...
def main():
 ...
 c=loadj(a.contract);host=loadj(a.host_lock);prof=loadj(a.profile_lock);man=loadj(a.candidate/'MANIFEST.json')
 refs=loadjl(a.candidate/'IDENTIFYING_COVISIBLE.token_reference.private.jsonl');idx,wseq,wins=schedule_for(man,a.seed,a.arm);ordered=[refs[i] for i in idx]
 import torch
 from torch.utils.data import Dataset,SequentialSampler
 from transformers import AutoModelForCausalLM,BitsAndBytesConfig,Trainer,TrainingArguments
 from peft import LoraConfig,get_peft_model,prepare_model_for_kbit_training
 det=configure_determinism(a.seed);opt=c['optimization'];profile=prof['selected_profile'];bnb=BitsAndBytesConfig(load_in_4bit=True,bnb_4bit_quant_type=opt['bnb_quant_type'],bnb_4bit_use_double_quant=opt['bnb_double_quant'],bnb_4bit_compute_dtype=torch.float16)
 ...
 args=TrainingArguments(output_dir=str(a.out/'trainer_tmp'),num_train_epochs=1,per_device_train_batch_size=1,gradient_accumulation_steps=8,learning_rate=opt['learning_rate'],lr_scheduler_type=opt['lr_scheduler_type'],warmup_ratio=0.0,weight_decay=opt['weight_decay'],max_grad_norm=opt['max_grad_norm'],optim=opt['optimizer'],fp16=True,bf16=False,gradient_checkpointing=opt['gradient_checkpointing'],logging_steps=1,save_strategy='no',do_eval=False,report_to=[],remove_unused_columns=False,dataloader_num_workers=0,dataloader_pin_memory=False,seed=a.seed,data_seed=a.seed,disable_tqdm=False)
 a.out.mkdir(parents=True);torch.cuda.reset_peak_memory_stats();t0=time.time();tr=ST(model=model,args=args,train_dataset=DS(),data_collator=collate);res=tr.train();torch.cuda.synchronize();elapsed=time.time()-t0
 if int(tr.state.global_step)!=144:raise RuntimeError(('GLOBAL_STEP',tr.state.global_step))
 final_sha,nfinal=hash_trainable_parameters(model);ad=a.out/'adapter';model.save_pretrained(ad,safe_serialization=True);files={p.relative_to(ad).as_posix():{'bytes':p.stat().st_size,'sha256':sha256_file(p)} for p in sorted(ad.rglob('*')) if p.is_file()};run={'schema':'cfe.dd2.train-run.v1','status':'TRAINING_COMPLETE__PAIR_UNQUALIFIED','arm':a.arm,'seed':a.seed,'initial_trainable_sha256':init_sha,'final_trainable_sha256':final_sha,'trainable_parameters':ntrain,'global_step':int(tr.state.global_step),'expanded_microbatches':len(ordered),'window_sequence_sha256':hashlib.sha256(json.dumps(wseq,separators=(',',':')).encode()).hexdigest(),'expanded_row_order_sha256':hashlib.sha256(json.dumps(idx,separators=(',',':')).encode()).hexdigest(),'contract_sha256':sha256_file(a.contract),'input_lock_sha256':sha256_file(a.lock),'runtime_seconds':elapsed,'train_result':res.metrics,'adapter_files':files,'peak_allocated_bytes':int(torch.cuda.max_memory_allocated()),'peak_reserved_bytes':int(torch.cuda.max_memory_reserved()),'determinism':det};dumpj(a.out/'RUN_MANIFEST.json',run);print(json.dumps(run,indent=2,sort_keys=True),flush=True)
