...
ARM_NARROW='COMPOSE_NARROW_BASIS'
ARM_IDENT='COMPOSE_IDENTIFYING_BASIS'
ARMS=(ARM_NARROW,ARM_IDENT)
DATASET_FOR_ARM={ARM_NARROW:'PREDICATE_NARROW_V12',ARM_IDENT:'PREDICATE_IDENTIFYING_V12'}
POLICY_DATASET='POLICY_Z_SHARED'
...
def verify_lock(root:Path,lock_path:Path):
 ...
 if lock.get('status')!='LOCKED_PRE_RUNTIME__NO_V12_SCIENTIFIC_TRAINING':raise SystemExit('INPUT_LOCK_STATUS_INVALID')
 fail=[]
...
def parse_json_output(raw:str):
 ...
 return None
def build_logical_schedule(seed:int):
 rng=random.Random(seed ^ 0xCFE12A);pred=list(range(72));pol=list(range(72));rng.shuffle(pred);rng.shuffle(pol);sched=[]
 for w in range(18):
  members=[('P',x) for x in pred[w*4:(w+1)*4]]+[('Q',x) for x in pol[w*4:(w+1)*4]];wr=random.Random((seed<<8)^w^0x12);wr.shuffle(members);sched.extend(members)
 if len(sched)!=144:raise RuntimeError('SCHEDULE_LENGTH')
 for w in range(18):
  window=sched[w*8:(w+1)*8]
  if sum(k=='P' for k,_ in window)!=4 or sum(k=='Q' for k,_ in window)!=4:raise RuntimeError(('WINDOW_MIX',w))
 return sched
