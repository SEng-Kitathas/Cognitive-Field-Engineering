...
def probe(snapshot,profile,contract,ref):
 ...
 det=configure_determinism(91320260831);torch.cuda.empty_cache();torch.cuda.reset_peak_memory_stats();o=contract['optimization'];bnbc=BitsAndBytesConfig(load_in_4bit=True,bnb_4bit_quant_type=o['bnb_quant_type'],bnb_4bit_use_double_quant=o['bnb_double_quant'],bnb_4bit_compute_dtype=torch.float16)
...
def main():
 ...
 refs=[]
 for name in [PREDICATE_DATASET,POLICY_DATASET]:refs.extend(loadjl(a.candidate/(name+'.token_reference.private.jsonl')))
 ...
 rep={'schema':'cfe.v13.optimizer-interference-repeatability.v1','status':'REPEATABILITY_PASS' if not mism else 'REPEATABILITY_FAIL','seed':91320260831,'reference_id':ref['id'],'reference_tokens':ref['tokens'],'required_equal_witnesses':keys,'mismatches':mism,'runs':runs,'input_lock_sha256':sha256_file(a.lock),'host_lock_sha256':sha256_file(a.host_lock)};dumpj(a.out/'REPEATABILITY_QUALIFICATION.json',rep)
 if mism:raise SystemExit(2)
 pl={'schema':'cfe.v13.optimizer-interference-profile-lock.v1','status':'PROFILE_INHERITED_AND_REPEATABILITY_QUALIFIED','input_lock_sha256':sha256_file(a.lock),'host_lock_sha256':sha256_file(a.host_lock),'selected_profile':profile,'inheritance_from_v12_profile_lock_sha256':host['v12_profile_lock_sha256'],'inheritance_reason':f"Same exact base/runtime; v13 max {host['v13_max_tokens']} <= v12 qualified max {host['v12_qualified_max_tokens']}; fresh two-run one-step repeatability PASS.",'repeatability_qualification_sha256':sha256_file(a.out/'REPEATABILITY_QUALIFICATION.json')};dumpj(a.out/'PROFILE_LOCK.json',pl);print(json.dumps({'repeatability':rep,'profile_lock':pl},indent=2,sort_keys=True),flush=True)
