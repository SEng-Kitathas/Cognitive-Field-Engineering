...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--project-root',type=Path,required=True);ap.add_argument('--lock',type=Path,required=True);ap.add_argument('--candidate',type=Path,required=True);ap.add_argument('--v10-run-root',type=Path,required=True);ap.add_argument('--v12-host-lock',type=Path,required=True);ap.add_argument('--v12-profile-lock',type=Path,required=True);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args();root=a.project_root.resolve();verify_lock(root,a.lock)
 if a.out.exists():raise SystemExit('REFUSE_OVERWRITE')
 a.out.mkdir(parents=True);v12h=loadj(a.v12_host_lock);v12p=loadj(a.v12_profile_lock);oldbase=loadj(a.v10_run_root/'host/base_snapshot_manifest.json');snap=Path(v12h['base_snapshot_path']);fail=[]
 ...
 for k in ['torch','torch_cuda','transformers','peft','bitsandbytes','numpy','cuda_available','gpu_name','gpu_total_memory_bytes','compute_capability']:
  if crit[k]!=v12h['runtime_critical'][k]:raise SystemExit(f'CRITICAL_RUNTIME_DRIFT {k} current={crit[k]!r} v12={v12h["runtime_critical"][k]!r}')
 ...
 for name in [PREDICATE_DATASET,POLICY_DATASET]:
 ...
 newmax=max(v['max_tokens'] for v in replay.values());oldmax=int(v12h['v12_max_tokens'])
 if newmax>oldmax:raise SystemExit(('PROFILE_LENGTH_INHERIT_FAIL',newmax,oldmax))
 host={'schema':'cfe.v13.optimizer-interference-host-lock.v1','status':'HOST_REVERIFIED__TOKENIZER_REPLAY_PASS__PROFILE_REPEATABILITY_PENDING','input_lock_sha256':sha256_file(a.lock),'base_snapshot_manifest_sha256':v12h['base_snapshot_manifest_sha256'],'base_snapshot_path':str(snap),'hf_repo':v12h['hf_repo'],'hf_revision':v12h['hf_revision'],'snapshot_file_count':len(oldbase['files']),'snapshot_all_files_reverified':True,'runtime_critical':crit,'v12_host_lock_sha256':sha256_file(a.v12_host_lock),'runtime_tokenizer_exact_match':True,'tokenizer_replay':replay,'v12_qualified_max_tokens':oldmax,'v13_max_tokens':newmax,'profile_inheritance_length_gate':newmax<=oldmax,'inherited_selected_profile':v12p['selected_profile'],'v12_profile_lock_sha256':sha256_file(a.v12_profile_lock),'laws':['SAME SOURCE != SAME EXECUTABLE ENVIRONMENT','OLD PROFILE FIT != NEW LOCK REPEATABILITY','TOKEN REFERENCE PRESENT != LIVE TOKENIZER MATCH']};dumpj(a.out/'HOST_LOCK.json',host);dumpj(a.out/'RUNTIME_ENVIRONMENT_CRITICAL.json',crit);print(json.dumps(host,indent=2,sort_keys=True),flush=True)
