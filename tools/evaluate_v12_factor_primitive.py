...
def main():
 ...
 for name,fn,maxnew in jobs:
  ...
  if name=='PREDICATE_DIRECT':
   m['by_truth']={str(z).lower():metric([r for r in rows if r['expected']['condition_z']==z]) for z in [False,True]};m['by_support']={k:metric([r for r in rows if r['support_bucket']==k]) for k in sorted({r['support_bucket'] for r in rows})}
  ...
  allmetrics[name]=m
 manifest={'schema':'cfe.v12.factor-primitive-eval.v1','status':'EVALUATION_EXECUTED__SCIENTIFIC_EFFECT_UNQUALIFIED','arm':run['arm'],'seed':run['seed'],'input_lock_sha256':sha256_file(a.lock),'host_lock_sha256':sha256_file(a.host_lock),'contract_sha256':sha256_file(a.contract),'producer_run_manifest_sha256':sha256_file(a.run_manifest),'candidate_manifest_sha256':sha256_file(a.candidate/'MANIFEST.json'),'metrics':allmetrics,'runtime_seconds':time.time()-t0,'peak_allocated_bytes':int(torch.cuda.max_memory_allocated()),'peak_reserved_bytes':int(torch.cuda.max_memory_reserved()),'results_sha256':result_hashes,'interpretation_ceiling':['Paired-seed aggregate required before composition claim.','Composed action must be interpreted with predicate-direct and policy-direct readouts.','Identifying CFE specialization only.']};dumpj(a.out/'EVAL_MANIFEST.json',manifest);print(json.dumps(manifest,indent=2,sort_keys=True),flush=True)
