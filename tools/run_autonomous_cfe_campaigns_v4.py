...
def build_corpus()->list[Chunk]:
    ...
    for root in (SEALED_PARENT,HOST_QUAL):
        ...
        for p in root.rglob("*"):
            if p.is_file() and p.suffix.lower() in TEXT_SUFFIXES and ".venv" not in str(p).lower():paths.append(p)
    out=[]
    for p in paths:
        txt=read_text(p)
...
def divergence(question:str,evidence:str)->tuple[dict[str,str],dict[str,Any],str]:
    system="You are R2 divergence pressure only. No truth/promotion authority. Produce one hostile OARR challenge and one Loop+ adjacent branch from supplied CFE evidence. Do not propose executing downloads/training. Do not state either branch as fact. Return exactly: OARR=<concrete rival/counterexample/removal> || LOOP=<adjacent plausible branch>."
    raw,meta=call(DIV_SERVER,system,f"QUESTION: {question}\nEVIDENCE:\n{evidence[:3600]}",90,0.35)
    f=parse_fields(raw,["OARR","LOOP"])
    if not f.get("OARR") or not f.get("LOOP") or f["OARR"].upper() in {"NONE","N/A"} or f["LOOP"].upper() in {"NONE","N/A"}:
        raw2,meta2=call(DIV_SERVER,system,"REFORMAT/REPAIR. Both fields must be substantive.\nQUESTION: "+question+"\nEVIDENCE:\n"+evidence[:3000]+"\nRAW:\n"+raw[:1000],100,0.2)
        f=parse_fields(raw2,["OARR","LOOP"]);raw=raw+"\n--REPAIR--\n"+raw2;meta={"first":meta,"repair":meta2}
    if not f.get("OARR") or not f.get("LOOP"):raise RuntimeError("divergence fields missing")
    return f,meta,raw
...
def main()->int:
    ...
    seed_path=PROJECT/"state"/"qualified_campaign_seed.json";seed=json.loads(seed_path.read_text(encoding="utf-8"));first=normalize_q(str(seed["question"]))
    manifest={"run_id":RUN_ID,"campaigns":3,"passes_per_campaign":20,"planned_qualified_passes":60,"question_authoring":"MODEL_CHAIN_ONLY_NO_HAND_AUTHORED_PASS_QUESTIONS","seed":seed,"seed_sha256":sha256_bytes(seed_path.read_bytes()),"generator_model":str(GEN_MODEL),"generator_sha256":GEN_SHA,"divergence_model":str(DIV_MODEL),"divergence_sha256":DIV_SHA,"runtime":str(RUNTIME),"runtime_sha256":RUNTIME_SHA,"servers":{"generator":GEN_SERVER,"divergence":DIV_SERVER},"network_downloads":"NONE","sealed_parent":str(SEALED_PARENT),"sealed_parent_mutation_allowed":False,"corpus_chunks":len(corpus),"source_classes":sorted(all_classes),"sop_sources":[str(p) for p in SOP_SOURCES],"rails":RAILS,"qualification":"DIVERGENCE_THEN_STRONG_SYNTHESIS_THEN_STRONG_SOURCE_AUDIT_EACH_PASS_PLUS_CSC"}
