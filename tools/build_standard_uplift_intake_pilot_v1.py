...
def heuristic_invariants(messages: list[dict[str, Any]], interaction: str, lanes: list[str]) -> dict[str, Any]:
    cfe = []
    lhit = []
    users = sum(1 for m in messages if m.get("role") == "user")
    ...
    return {"cfe": sorted(set(cfe)), "lhit": sorted(set(lhit)), "tags_state": "HEURISTIC" if cfe or lhit else "UNASSESSED"}
...
def make_atom(spec: dict[str, Any], idx: int, row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "invariants": heuristic_invariants(messages, interaction, spec["lane"]),
...
def main() -> None:
    ...
    summary = {
        "schema": "cfe.standard-uplift.intake-pilot.v1",
