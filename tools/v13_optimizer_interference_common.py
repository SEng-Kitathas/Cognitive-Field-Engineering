...
ARM_MIXED='LOCAL_MIXED'
ARM_SEPARATED='WINDOW_SEPARATED'
ARMS=(ARM_MIXED,ARM_SEPARATED)
PREDICATE_DATASET='PREDICATE_IDENTIFYING_V12'
...
def verify_lock(root:Path,lock_path:Path):
 ...
 if lock.get('status')!='LOCKED_PRE_RUNTIME__NO_V13_SCIENTIFIC_TRAINING':raise SystemExit('INPUT_LOCK_STATUS_INVALID')
 fail=[]
...
def parse_json_output(raw:str):
 ...
 return None
def load_frozen_schedule(candidate:Path,seed:int,arm:str):
 man=loadj(candidate/'MANIFEST.json');node=man['schedules'][str(seed)][arm];schedule=[tuple(x) for x in node['schedule']];calc=sha256_bytes(json.dumps(node['schedule'],separators=(',',':')).encode())
 if calc!=node['sha256']:raise RuntimeError(('FROZEN_SCHEDULE_SHA_MISMATCH',seed,arm,calc,node['sha256']))
 if len(schedule)!=144:raise RuntimeError(('SCHEDULE_LENGTH',seed,arm,len(schedule)))
 return schedule,node['sha256']
