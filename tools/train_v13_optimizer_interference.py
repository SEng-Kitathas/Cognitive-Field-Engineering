...
def main():
 ...
 pred=loadjl(a.candidate/(PREDICATE_DATASET+'.token_reference.private.jsonl'));pol=loadjl(a.candidate/(POLICY_DATASET+'.token_reference.private.jsonl'))
 if len(pred)!=72 or len(pol)!=72:raise SystemExit('TOKENREF_COUNT')
 sched,sched_sha=load_frozen_schedule(a.candidate,a.seed,a.arm);ordered=[pred[ix] if kind=='P' else pol[ix] for kind,ix in sched]
 # fail closed on window geometry
 wins=[sched[w*8:(w+1)*8] for w in range(18)]
 if a.arm==ARM_MIXED:
  if not all(sum(k=='P' for k,_ in win)==4 and sum(k=='Q' for k,_ in win)==4 for win in wins):raise RuntimeError('MIXED_WINDOW_CONTRACT')
 else:
  kinds=[]
  for win in wins:
   ks={k for k,_ in win}
   if len(ks)!=1:raise RuntimeError('SEPARATED_WINDOW_MIXED')
   kinds.append(next(iter(ks)))
  if kinds.count('P')!=9 or kinds.count('Q')!=9 or any(kinds[i]==kinds[i+1] for i in range(17)):raise RuntimeError(('SEPARATED_WINDOW_CONTRACT',kinds))
 import torch
